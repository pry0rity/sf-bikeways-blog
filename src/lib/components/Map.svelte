				...
				.then((routes) => {
						...
						if (layer) {
							if (isHovering) {
								...
								if (showControls) {
									...
									if (route) {
										const popup = L.popup()
											.setLatLng(route.geojson.geometry.coordinates[0])
											.setContent(
												`
                    <div class="p-2">
                      <h3 class="font-medium">${route.title}</h3>
                  ...
                  `
					...
					const allCoords = routes.flatMap(
						(route: any) => route.geojson?.geometry?.coordinates || []
					);
					if (allCoords.length > 0) {
						const bounds = L.latLngBounds(allCoords.map(([lng, lat]: number[]) => [lat, lng]));
...
<div bind:this={mapContainer} class="w-full {className}" style="height: {height}"></div>
